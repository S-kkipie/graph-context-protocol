...
La Figura~\ref{fig:pipeline} resume este procedimiento de extremo a extremo: una definici\'on de
escenario y un modelo de chat inyectado instancian ambos brazos bajo la misma l\'ogica de agente, y el
or\'aculo y el recolector de m\'etricas son compartidos, de modo que solo la capa de interoperabilidad
difiere entre las dos corridas de una misma tripleta (modelo, escenario, semilla).
...
\node[box, below right=of def, xshift=-6mm] (a2a) {Brazo \ata{}: servidores A2A\\en localhost, punto a punto};
...
\caption{Procedimiento de evaluaci\'on por corrida. Solo la f\'abrica de herramientas por par y el
alojamiento de nodos difieren entre los brazos \gcp{} y \ata{}; la l\'ogica de agente, el modelo, la
tarea y la semilla se mantienen fijos.}
\label{fig:pipeline}
\end{figure}
...
La Tabla~\ref{tab:struct} y la Figura~\ref{fig:coupling} muestran la curva estructural: \gcp{} crece
linealmente, \ata{} cuadr\'aticamente ---$9\times$ en $n{=}10$, $49\times$ en $n{=}50$. El esfuerzo de
integraci\'on es constante $1$ para \gcp{} y $2n$ para \ata{}, de modo que la brecha se ampl\'ia con
cada nodo. Id\'entico sin importar el modelo: la curva la fija la topolog\'ia ---$C_{\gcp{}}(n)=n$ vs.\
$C_{\ata{}}(n)=n(n{-}1)$--- no el comportamiento del modelo.
...
presupuesto de prompt de herramientas (Tabla~\ref{tab:disc}, Figura~\ref{fig:overhead}). \gcp{}
mantiene ambos planos ($1$ mensaje, $72$ tokens); \ata{} base llega a $32$ mensajes y $1782$ tokens de
prompt de herramientas en $n{=}32$, una brecha de $25\times$ en el prompt. Id\'entico sin importar el
modelo, ya que estos valores los fija la construcci\'on del prompt, no el comportamiento del modelo.
...
\paragraph{Las limitaciones de recursos acotan el alcance de la evaluaci\'on.} Este estudio no recibi\'o
financiamiento externo y se realiz\'o como una iniciativa independiente (v\'ease la declaraci\'on de
Financiamiento); el alcance reportado refleja ese l\'imite de recursos y no una elecci\'on
metodol\'ogica. Las semillas de los modelos de peso abierto, alojados localmente, est\'an acotadas por
el costo de tiempo de c\'omputo (wall-clock), ya que cada semilla requiere un ciclo completo de agente
ReAct de varios pasos contra un modelo servido localmente, repetido por modelo, escenario y brazo; esto
mantiene al brazo de peso abierto en 3--5 semillas en lugar de las 20--30 usadas en dos de los tres
niveles de Gemini alojados. Las semillas de los modelos alojados, a su vez, est\'an acotadas por el
uso pagado de la API, raz\'on por la cual solo se evalu\'o una familia de frontera alojada y por la
cual el tercer nivel de Gemini se detiene en 15 semillas en lugar de 30. Estas limitaciones no
amenazan las afirmaciones deterministas/arquitect\'onicas (acoplamiento, descubrimiento, sobrecosto de
prompt de herramientas, procedencia, contenci\'on), que se sostienen por construcci\'on
independientemente del tama\~no de la muestra; s\'i acotan cu\'an fuertemente pueden leerse las
columnas conductuales preliminares (tokens, latencia, fuga, \'exito), y una r\'eplica realizada con un
presupuesto mayor de c\'omputo o de API deber\'ia esperarse que reduzca, no que revierta, los tama\~nos
de efecto peque\~nos ya reportados. El trabajo relacionado citado en la Secci\'on~\ref{sec:related}
(p.\ ej., \citet{zhang2024agentprune}, \citet{mao2026delm}) eval\'ua a escala de producci\'on o con un
presupuesto mayor; la comparaci\'on ofrecida aqu\'i es un complemento controlado por equidad pero
acotado por recursos frente a esa literatura, no un reemplazo equivalente.
