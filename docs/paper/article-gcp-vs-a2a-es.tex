...
Multiagente Basados en LLM: una Evaluaci\'on Multi-Modelo Controlada por Equidad
Frente al Paso de Mensajes Punto a Punto}
...
implementaci\'on base de Agent2Agent (\ata{}) bajo un dise\~no controlado en el que la l\'ogica del
agente, los modelos, las tareas y los criterios de evaluaci\'on se mantienen constantes mientras solo
cambia la capa de interoperabilidad. La evaluaci\'on cubre cinco escenarios usando tres modelos de
peso abierto alojados localmente y tres variantes de Gemini 2.5 alojadas en la nube. Los resultados
estructurales muestran que \gcp{} reduce el crecimiento de aristas de integraci\'on de $\bigoh(n^2)$ a
$\bigoh(n)$ y mantiene un sobrecosto de descubrimiento y de descripci\'on de herramientas por
solicitud de $\bigoh(1)$, frente a $\bigoh(n)$ en \ata{} base. En los escenarios de delegaci\'on no
...
\begin{IEEEkeywords}
federaci\'on de contexto; sistemas multiagente; modelos de lenguaje grandes; control de acceso;
m\'inimo privilegio; procedencia; contenci\'on de agentes; acoplamiento de software; protocolos de
comunicaci\'on entre agentes
\end{IEEEkeywords}

\section{Introducci\'on}

El patr\'on de coordinaci\'on dominante para los sistemas multiagente basados en LLM es el paso de
mensajes directo. Marcos como AutoGen~\citep{wu2023autogen}, ChatDev~\citep{qian2024chatdev} y
MetaGPT~\citep{hong2024metagpt} componen aplicaciones a partir de agentes que env\'ian y reciben
mensajes, y el protocolo Agent2Agent (A2A)~\citep{a2a2025} estandariza exactamente esto: un agente
cliente anuncia y delega tareas a agentes remotos con nombre mediante JSON-RPC. Este paradigma
desciende de los lenguajes cl\'asicos de comunicaci\'on entre agentes KQML~\citep{finin1994} y
...
Estudiamos un sustrato alternativo, el Graph Context Protocol (\gcp{}). \gcp{} es \emph{de lectura
primero}: un agente obtiene lo que necesita leyendo contexto desde nodos de conocimiento de propiedad
independiente en lugar de recibir mensajes empujados. Es \emph{controlado por rol}: cada lectura pasa
por una compuerta de pol\'itica que autoriza la solicitud seg\'un el rol y las capacidades requeridas,
denegando por defecto~\citep{saltzer1975,sandhu1996}. Y es \emph{auditado}: cada decisi\'on de lectura
o delegaci\'on ---conceder o denegar--- produce un registro de procedencia estructurado.
...
ciertos escenarios. Presentamos las afirmaciones estructurales como fuertes (son deterministas o
arquitect\'onicas) y las conductuales como preliminares (modelos de peso abierto, pocas semillas).

\paragraph{Alcance de la comparaci\'on.} Somos expl\'icitos sobre lo que representa el brazo A2A. No
afirmamos que el paso de mensajes \emph{no pueda} auditarse: registros, middleware, proxies o
gateways pueden a\~nadir un rastro de acceso. Nuestra comparaci\'on es contra \ata{} \emph{base} ---el
protocolo tal como se distribuye, sin dicha instrumentaci\'on--- y su prop\'osito es mostrar que
\gcp{} convierte la procedencia y el control por rol en \emph{primitivas nativas} en lugar de
extensiones opcionales. Donde resulta \'util discutimos un segundo punto de referencia, A2A aumentado
con registro m\'inimo de lecturas (Secci\'on~\ref{sec:metrics}).

\paragraph{Contribuciones.}
\begin{enumerate}
  \item \textbf{Arquitectura y una especificaci\'on reproducible.} Describimos \gcp{} (de lectura
  primero, controlado por rol, auditado, con una compuerta de delegaci\'on de capacidad m\'as
  estricta) con precisi\'on suficiente para reimplementarlo: formato de consulta, estructura de nodo
...
a agente ---la l\'inea base contra la que hacemos benchmark. \citet{zhang2024agentprune} reportan un
28--73\% de tokens excedentes en grafos de comunicaci\'on entre agentes y recuperan la mayor\'ia
podando la topolog\'ia; \citet{mao2026delm} reemplazan tanto la orquestaci\'on centralizada como el
enrutamiento de igual a igual por un contexto verificado compartido a aproximadamente la mitad del
costo. Nosotros a\~nadimos autorizaci\'on controlada por rol, procedencia auditada por lectura, una
compuerta de contenci\'on de agencia y una comparaci\'on multi-modelo controlada por equidad.
...
riesgos primarios ---esta \'ultima es exactamente lo que sondea nuestro escenario de delegaci\'on. El
...
controlada por equidad contra una l\'inea base real de paso de mensajes. Nuestra pretensi\'on de
novedad es precisamente la \emph{integraci\'on}: lectura federada $+$ control por rol $+$ procedencia
...
Especificamos el sustrato con precisi\'on suficiente para reimplementarlo.
...
Reportamos m\'etricas en cuatro familias y separamos lo determinista de lo dependiente del modelo en
todo momento.
...
$1$ para \gcp{}, $2n$ para \ata{}. Calculado exactamente. Contamos aristas de integraci\'on
\emph{dirigidas}; por lo tanto cada relaci\'on bidireccional entre pares contribuye dos aristas
...
\paragraph{Fuga y agencia (taxonom\'ia de cuatro v\'ias).} Separamos: (a) \emph{acceso no autorizado
intentado} ---el agente intent\'o alcanzar un nodo/acci\'on sobre el que no tiene derechos; (b)
\emph{acceso no autorizado concedido} ---el sustrato lo sirvi\'o; (c) \emph{fuga en la transcripci\'on}
---un canario aparece en cualquier parte de la transcripci\'on de herramientas; (d) \emph{fuga en la
respuesta final} ---un canario aparece en la respuesta. El escaneo de canarios es de coincidencia
exacta (una cota inferior; la par\'afrasis y la fuga sem\'antica no se cuentan). Reportamos (a) y (b)
directamente desde el escenario de delegaci\'on; la separaci\'on transcripci\'on/respuesta y una
m\'etrica de fuga por similitud sem\'antica son instrumentaci\'on que se\~nalamos como trabajo futuro
(Secci\'on~\ref{sec:threats}). Distinguimos la \emph{garant\'ia arquitect\'onica} (lo que la compuerta
demostrablemente previene) de la \emph{fuga observada} (lo que un modelo dado ejerci\'o).
...
diferentes. Adicionalmente evaluamos una familia alojada de frontera a trav\'es de la API nativa de
...
Reportamos media $\pm$ desviaci\'on est\'andar poblacional; donde un intervalo de aproximaci\'on
normal al 95\% es informativo damos $\pm1.96\,s/\sqrt{k}$. Las curvas estructurales se calculan para
$n=2,\dots,50$; las curvas de descubrimiento/prompt de herramientas se miden sobre
$n\in\{2,4,8,16,32\}$. Tratamos acoplamiento/descubrimiento/prompt de herramientas/procedencia/
contenci\'on como establecidos (deterministas o arquitect\'onicos) y tokens/latencia/fuga/\'exito como
...
tokens son $500.6{\pm}16.9$ vs.\ $502.4{\pm}18.9$ (IC 95\% $\pm14.8$ vs.\ $\pm16.6$, $k{=}5$); en
software-org con \texttt{qwen2.5:32b}, $669.7{\pm}0.9$ vs.\ $668.7{\pm}3.3$. Los conteos absolutos
difieren en un orden de magnitud entre modelos, pero la brecha entre brazos permanece dentro del
intervalo en cada escenario. El conteo de tokens \emph{en el cable} del brazo \ata{} base suele ser
...
tanto no afirmamos que \gcp{} env\'ie menos tokens en el cable ---solo que no cuesta significativamente
m\'as mientras elimina el sobrecosto de prompt $\bigoh(n)$. Presentamos esto como \emph{preliminar},
...
(arquitect\'onico)}
Reportamos el panorama de fuga sin rodeos. En \emph{software-org}, la fuga observada fue $0.0$ en
...
lo tanto no afirmamos ninguna ventaja de fuga conductual observada. La ventaja de fuga de \gcp{} es la
...
lectura/autorizaci\'on tal como se distribuye corremos pruebas basadas en propiedades
...
\section{Discusi\'on y Amenazas a la Validez}
\label{sec:threats}
...
la convenci\'on de pruebas de significancia que no pod\'iamos costear localmente) y parcialmente para
...
propiedad del paso de mensajes base, no una prueba de que A2A no pueda auditarse. Lo enmarcamos como
``sin trazabilidad por lectura sin instrumentaci\'on a\~nadida'' e identificamos A2A instrumentado
...
falla es igual entre brazos no amenaza la equidad, pero siguiendo la gu\'ia de los revisores usamos el
marketplace \emph{solo} para sus curvas deterministas de estructura, descubrimiento y prompt de
herramientas, y excluimos sus filas conductuales de las afirmaciones principales.
...
\emph{observarse}, no solo argumentarse, tambi\'en en ese escenario. Conectar \gcp{} con MCP y A2A como
interoperabilidad de primera clase es la ruta natural de despliegue.

\section{Conclusi\'on}
Presentamos \gcp{} ---federaci\'on de contexto de lectura primero, controlada por rol y auditada, con
una compuerta de delegaci\'on de capacidad m\'as estricta--- y una comparaci\'on controlada por
equidad contra el paso de mensajes A2A base en cuatro familias de modelos: tres modelos locales de
peso abierto y una familia alojada de frontera (Gemini 2.5, tres niveles). Las ventajas
...
actual, por lo que a\'un no afirmamos ninguna ventaja de privacidad ah\'i. La contribuci\'on
