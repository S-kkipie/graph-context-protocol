...
significancia sobre las diferencias de tokens y latencia. Quedan dos capacidades
diseñadas pero no evaluadas: una capa de \emph{delegación de tareas} con capacidad
más estricta sobre la misma ruta de lectura primero, y un \emph{modelo formal con
una propiedad ejecutable de no fuga} (un principal nunca lee más allá de la política
bajo políticas y principales aleatorios), que elevaría el argumento de fuga de
medido a probado. Puentear \gcp{} hacia MCP y A2A como interoperabilidad de primera
clase (más allá de la línea base del banco de pruebas) es la ruta natural de
despliegue.
