...
implementation under a controlled design in which agent logic, models, tasks, and evaluation criteria
are held constant while only the interoperability layer changes. The evaluation covers five scenarios
using three locally hosted open-weight models and three hosted Gemini 2.5 variants. Structural results
show that \gcp{} reduces integration-edge growth from $\bigoh(n^2)$ to $\bigoh(n)$ and maintains
$\bigoh(1)$ per-request discovery and tool-description overhead compared with $\bigoh(n)$ for base
\ata{}. In unauthorized delegation and forced-access scenarios, \gcp{} denied every attempted
