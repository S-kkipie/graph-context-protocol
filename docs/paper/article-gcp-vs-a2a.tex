...
% of paper.tex; rebuilds the empirical section from the four-model run set
% (llama3.1:8b, qwen3:4b x2, qwen2.5:32b, 2026-08-11); adds the delegation
...
\begin{abstract}
\textbf{Problem.} Multi-agent systems built on large language models (LLMs) coordinate
predominantly by \emph{point-to-point message-passing}, as standardized by Google's Agent2Agent
(A2A) protocol. This paradigm couples every interacting pair, and---in its base form, without
additional instrumentation---produces no per-read access trail. \textbf{Method.} We evaluate an
alternative substrate, the Graph Context Protocol (\gcp{}): agents \emph{read} context from
independently owned knowledge nodes through a role gate that authorizes each read and records a
provenance entry. We compare the two under a fairness invariant---identical agent brain, model,
and task; only the interop layer swapped---over four scenarios (marketplace, software
organization, cross-owner supply chain, delegation) and four open-weight models
(\texttt{llama3.1:8b}, \texttt{qwen3:4b}, \texttt{qwen2.5:32b}), 3--5 seeds per arm.
\textbf{Results.} \gcp{} offers \emph{structural} advantages in coupling and traceability that are
model-independent: integration edges grow $\bigoh(n)$ vs.\ $\bigoh(n^2)$ ($50$ vs.\ $2450$ at
$n{=}50$), and per-request discovery and tool-prompt overhead stay constant for \gcp{} while
growing linearly for the base A2A arm. In the delegation scenario, \gcp{} refused every
unauthorized action (containment $1.00$) while the base A2A arm refused none and executed the
action under three of four models. Provenance is native to \gcp{} and requires added instrumentation
under message-passing. We find \emph{preliminary} evidence that \gcp{} does not increase---and in
some scenarios reduces---cost, at equal task success. \textbf{Limitations.} Behavioral results use
open-weight models and few seeds; canary leakage \emph{tied} across arms in the supply-chain
scenario, so no privacy advantage is demonstrated there. \textbf{Contribution.} The defensible
novelty is not read-first access, role gating, or auditing in isolation---each exists---but their
\emph{integration} (federated read $+$ role control $+$ per-read provenance) and a
fairness-controlled, multi-model comparison against a real message-passing baseline.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{A multi-model empirical head-to-head} over four scenarios and four open-weight
  models, separating deterministic from model-dependent results (Section~\ref{sec:results}).
...
\begin{table}[t]
\centering
\caption{Property comparison. ``ext.'' $=$ achievable only with additional external instrumentation.}
\label{tab:props}
...
\midrule
agent$\leftrightarrow$agent            & yes & no  & yes \\
read-first                             & no  & yes & yes \\
cross-owner boundary                   & yes & no  & yes \\
role/capability gating                 & coarse & no & yes \\
per-read provenance (native)           & ext. & ext. & yes \\
deny-by-default                        & no  & no  & yes \\
stricter-capability delegation         & no  & n/a & yes \\
\bottomrule
\end{tabular}
\end{table}
...
per arm), \texttt{qwen3:4b} (3 seeds per arm, two independent run sets), \texttt{qwen2.5:32b} (3 seeds
per arm). The harness is CI-gated, throttles, and retries on transient errors. We report mean $\pm$
...
for \ata{}, so the gap widens with every node. Identical across all four models.
...
messages and $1782$ tool-prompt tokens at $n{=}32$, a $25\times$ prompt gap. Identical across all four
models, since these are set by prompt construction, not model behavior.
...
Across all four scenarios, all four models, every seed, provenance completeness is $1.0$ for \gcp{}
...
\begin{table}[t]
\centering
\caption{Delegation, four-way agency metrics by model. att.\ $=$ attempted unauthorized;
grant\ $=$ granted unauthorized (executed); cont.\ $=$ containment.}
\label{tab:deleg}
...
run property-based tests: P1 (end-to-end no-content-leak) asserts that for randomly generated
$(policy,principal)$ pairs an unauthorized query never returns a sentinel canary and is marked
\texttt{denied}, while an authorized one does return it (liveness, so the test is not vacuous); P2
(authorize soundness) checks the shipped gate equals the reference predicate; P3 (role-inheritance
soundness) checks effective-capability computation. All pass at 1000 runs. The canary metric
\emph{measures} leakage in scenarios; the property suite \emph{proves} the gate is sound over the input
space. Together: the property shows the gate is correct; the canary confirms it is on the hot path.

\section{Discussion and Threats to Validity}
\label{sec:threats}

\paragraph{Four models, but open-weight and few seeds.} The behavioral results span four open-weight
models over two size classes ($4$B--$32$B), removing the single-model threat, but seed counts are
3--5 and all models are locally hosted. The deterministic and architectural columns do not depend on
model or seed and are robust; the token/latency/leakage/success columns are \emph{preliminary} and
should be replicated with 20--30 seeds, confidence intervals, and significance tests on the deltas,
and on at least one frontier hosted model.
