...
capability ids). A principal $K$ holds an optional role; its effective capability set
$\mathit{effective}(K)$ is the union of $K$'s own flat capabilities ($K.\textit{capabilities}$) and
those inherited transitively through the role chain (own-first precedence on duplicate capability
ids), the role part computed by \texttt{getEffectiveCapabilities}.
