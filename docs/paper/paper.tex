...
\author{%
Graph Context Protocol Project\\
\textit{AI-DO}\\
Email: diego@workin2.com
}
...
regardless of whether a given model happens to probe the boundary.
...
the token and latency deltas. Two designed-but-unevaluated capabilities remain: a stricter-capability
\emph{task-delegation} layer on the same read-first path, and a \emph{formal model with an executable
no-leak property} (a principal never reads beyond policy under randomized policies and principals),
which would upgrade the leakage argument from measured to proven. Bridging \gcp{} to MCP and A2A as
first-class interop (beyond the benchmark baseline) is the natural deployment path.
