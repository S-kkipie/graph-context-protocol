...
- **Vitest project names** are mixed in this repo (`@ai-do/core` vs `@graph-context-protocol/adapters`); `nx test` accepts the Nx project name. Use `pnpm nx show projects` if a target name is uncertain.

## Execution Notes (corrections discovered while running the plan)

The plan executed end-to-end; the following deviations from the verbatim plan text were necessary and are reflected in the committed code (not in the code blocks above). A future re-run should prefer these:

1. **nx project names ≠ vitest names.** The nx project for core is **`core`** (resolves to `@graph-context-protocol/core`), not `@ai-do/core`. Use `core` / `server` / `@graph-context-protocol/scenario` with `nx`.
2. **Target topology.** Library packages expose only `typecheck` + `test` nx targets (no `build`); only the **apps** have `build`. `lint` is the root **`pnpm lint`** (biome) script — there is **no** nx `lint` target, so `nx run-many -t lint …` is a no-op. The real gate is: `pnpm nx run-many -t typecheck test` (libs) + `OPENROUTER_API_KEY=<dummy> pnpm nx run-many -t build` (apps) + `pnpm lint` (biome).
3. **Task 4 — zod v4 defaults.** `accessPolicy` must use `.prefault({})`, not `.default({})`: in zod v4 `.default({})` returns the literal `{}` without applying inner field defaults, which breaks the "defaults when omitted" case. The round-trip test also needs `metadata: {}` (core's `AccessPolicyDescriptorSchema` requires it) — implemented by constructing metadata via `createMetadataWithAccessPolicy`.
4. **Task 6 — async-throw semantics.** `runManifest` must be a **sync** function that runs `NodeManifestSchema.parse(...)` synchronously and then returns the launch promise. As an `async function`, a pre-`await` throw rejects the promise rather than throwing, so `expect(() => runManifest({nodes:[]})).toThrow()` would fail.
5. **Factory default graphId.** `createGcpNode` derives a default graph id via a tested `resolveGraphId(nodeId, graphId?)` helper that strips a leading `node:` (so an omitted `graphId` yields `graph:researcher`, not `graph:node:researcher`). Commit `fix(scenario): derive default graphId…`.
6. **Pre-existing `server:typecheck` fix (not in original plan).** server's nx config manually overrode `typecheck` with `tsc --build --noEmit`, which fails TS6310 against composite, declaration-emitting refs. Removed the override so it inherits the plugin default (`tsc --build --emitDeclarationOnly`). Commit `chore(config): fix server typecheck target…`.
7. **Biome formatting.** The plan's verbatim single-line snippets exceeded biome's line width; M0 files were formatted with `biome check --write`. Remaining repo `pnpm lint` failures are **pre-existing** debt in non-M0 files (component `.tsx`, generated `index.d.ts`, older server specs) plus whole-file tsconfig formatting — out of M0 scope.
